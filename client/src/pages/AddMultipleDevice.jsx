...
const AddMultipleDevices = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/devices/create-bulk-devices", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      setToastMessage("Devices uploaded successfully.");
      ...
      if (error.response) {
        ...
        setErrorMessages("An unexpected error occurred while uploading devices.");
      }
    }
  };
  ...
  return (
    <>
      <Nav />
      <p className='m-10 text-white cursor-pointer hover:text-gray-300 w-36' onClick={handleBack}> &lt; Back to devices</p>
      <div className="m-10 p-11 h-[330px] w-96 bg-slate-900 absolute rounded-2xl top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col gap-7">
        <h3 className="text-center text-2xl font-sans text-white">Add Multiple Devices</h3>
